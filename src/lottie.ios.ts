import { Color, knownFolders, path, Utils } from '@nativescript/core';
import {
  autoPlayProperty,
  loopProperty,
  LottieViewBase,
...
export class LottieView extends LottieViewBase {
  ...
  [srcProperty.setNative](src: string) {
    if (!src) {
      ...
      this.nativeView.compatibleAnimation = CompatibleAnimation.alloc().initWithJson(
        src
      );
  ...
  public setOpacityValueDelegateForKeyPath(
    ...
    if (this.nativeView && value && keyPath && keyPath.length) {
      ...
      this.nativeView.setFloatValueForKeypath(
        value,
        CompatibleAnimationKeypath.alloc().initWithKeypath(keyPath.join('.'))
      );
    }
  ...
  public playAnimationFromProgressToProgress(
    ...
    if (this.nativeView) {
      ...
      if (this.completionBlock) {
        this.nativeView.playFromProgressToProgressCompletion(
          startProgress,
          endProgress,
