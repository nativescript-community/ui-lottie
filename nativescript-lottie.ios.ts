...
declare var UIViewContentModeScaleAspectFit: any;
...
export class LottieView extends LottieViewBase {
	...
	private createAnimationView(src: string) {
		...
		const newFrameHeight =
			typeof this.height === 'number'
				? this.height
				: this.getMeasuredHeight() > 0 ? this.getMeasuredHeight() / 2 : 150;
		const newFrameWidth =
			typeof this.width === 'number'
				? this.width
				: this.getMeasuredWidth() > 0 ? this.getMeasuredWidth() / 3 : 150;
	...
	public onMeasure(widthMeasureSpec: number, heightMeasureSpec: number) {
		...
		if (nativeView) {
			...
			const width =
				typeof this.width === 'number' ? this.width : measuredWidth / 3;
			const height =
				typeof this.height === 'number' ? this.height : measuredHeight / 2;
